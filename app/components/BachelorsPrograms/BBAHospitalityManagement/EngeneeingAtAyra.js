      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8">
        ...
        <div className="mt-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
          {yearJourney.map((item, index) => (
            ...
            >
              <h3 className="text-[#72D9FF] font-tthoves-bold text-[32px] md:text-[18px] leading-[1.15]">
                {item.title}
              </h3>
              <p className="mt-2 text-[#D9DDFF] font-tthoves-light text-[20px] md:text-[16px] leading-[1.25]">
                {item.description}
              </p>
            </div>
          ))}
