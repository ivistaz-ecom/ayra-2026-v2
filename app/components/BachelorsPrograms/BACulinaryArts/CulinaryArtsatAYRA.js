      ...
      <div className="container mx-auto px-4 md:px-0">
        ...
        <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-x-6 sm:gap-y-8 lg:gap-y-0">
          {yearJourney.map((item, index) => (
            ...
            >
              <h3 className="text-[#FFBCDB] font-tthoves-bold text-[15px] sm:text-[16px] md:text-[17px] lg:text-[18px] leading-[1.25]">
                {item.title}
              </h3>
              <p className="mt-2 text-[#FFFFFF] font-tthoves-light text-[13px] sm:text-[14px] md:text-[15px] lg:text-[16px] leading-[1.35]">
                {item.description}
              </p>
            </div>
          ))}
