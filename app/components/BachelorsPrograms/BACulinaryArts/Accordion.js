...
const YearBlock = ({
  ...
  return (
    <button
      type="button"
      onClick={onToggle}
      className="w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 focus-visible:ring-offset-2 focus-visible:ring-offset-[#744AC6]"
      aria-expanded={isOpen}
    >
      {/* Desktop: YEAR heading block, then (when open) two semester grid items */}
        ...
        <div className="grid md:grid-cols-[280px_1fr] gap-6 sm:gap-8 md:gap-20 items-center md:items-start mb-8 sm:mb-10 md:mb-14 text-left">
          ...
          <div className="pt-0 md:pt-4 mx-0 max-w-[720px] w-full">
            <p className="font-tthoves-light text-white text-[13px] sm:text-[14px] md:text-[18px] leading-relaxed">
              At AYRA, learning moves from foundational techniques to applied
              culinary practice.
            </p>
            <p className="mt-4 md:mt-6 font-tthoves-light text-white text-[13px] sm:text-[14px] md:text-[18px] leading-relaxed">
              Students build confidence through training labs, practical
              sessions, and real-world exposure rather than relying only on
              theoretical instruction.
            </p>
          </div>
        </div>

        <div className="w-full border border-dashed border-white/35 overflow-hidden">
